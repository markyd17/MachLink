...
A locally-hosted companion app for DCS World. It detects the aircraft
you're flying, shows startup/shutdown/weapons/flight-planning reference
material for that specific airframe, surfaces the real in-mission
briefing, and lets you ask follow-up questions — grounded only in the
reference data you've loaded, not free-generated by an LLM.
...
The app's job is to detect context, retrieve the matching content, and
display/search it — not to generate it.

Because Chuck's Guides is copyrighted by its author (free to download, not
licensed for me to bulk-copy into this repo), this project ships with an
**empty data set** and a **PDF import tool** that
...
For each airframe:
1. Download the PDF from Chuck's Guides — your own legally-downloaded copy.
...
Detection, retrieval, the kneeboard, and the Debrief feature (both phases)
are all functional. The aircraft *data* is empty aside from
